...
class CBDecomp(Decomposition):
    ...
    def get_question(self, statement, prompt, boost_ex, manifest, overwrite_manifest, saved_question):
        prompt_suffix = prompt(boost_ex)
        quesiton_prompt = f"\n{prompt_suffix}\n\nStatement: {{statement:}}\nQuestion:"
        question_pmp = quesiton_prompt.format(statement=statement)
        if self.use_question:
            answer = saved_question
        else:
            answer = get_response(
                question_pmp,
                manifest,
    ...
    def _run_decomp_single_data(self, test_data, boost_dfs, manifest, overwrite_manifest, run_limit=-1):
        ...
        for i, (ind, row) in tqdm(
            ...
            for j, boost_examples in enumerate(boost_dfs):
                all_prompts = []
                saved_question = self.saved_augmentations[i]["question"][j] if self.use_question else None
                question, question_final_prompt = self.get_question(
                    statement, questioner, boost_examples[0], manifest, overwrite_manifest, saved_question
                )
