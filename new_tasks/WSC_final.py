...
extract_relevant_phrase_examples = [
    pd.DataFrame([
        ...
        {
            "passage": "Andrew drove Oscar to school in the morning. Andrew dropped \"him\" off outside the entrance.",
            "extract": "phrase containing \"him\": dropped \"him\" off"
        },
        {
            "passage": "The teacher told the mother that \"she\" should be proud.",
            "extract": "phrase containing \"she\": \"she\" should be proud"
        }
    ]),
    pd.DataFrame([
        {
        ...
        {
            "passage": "The soccer players lost the game by one goal and \"they\" were upset.",
            "extract": "phrase containing \"they\": \"they\" were upset"
        },
        {
            "passage": "Jesus was not born in the inn because \"it\" had no room.",
            "extract": "phrase containing \"it\": \"it\" had no room"
        }
    ]),
    pd.DataFrame([
        {
        ...
        {
            "passage": "The child was given many toys on Chistmas. They loved \"them\".",
            "extract": "phrase containing \"them\": They loved \"them\""
        },
        {
            "passage": "John couldn't see the stage with Billy in front of him because \"he\" is so short.",
            "extract": "phrase containing \"he\": \"he\" is so short"
...
convert_reason_to_q_examples = [
    pd.DataFrame([
        ...
        {
            "input": "she wore a dress",
            "question": "Who wore a dress?"
        },
        {
            "input": "it fell on annie’s foot",
            "question": "What fell on Annie’s foot?"
        },
        {
            "input": "he yelled at his mother",
            "question": "Who yelled at his mother?"
        }
    ]),
    pd.DataFrame([
        {
...
answer_q_in_passage_examples = [
    pd.DataFrame([
        ...
        {
            "passage": "The dog chased the cat into the house.",
            "question": "Who did the dog chase?",
            "answer": "The cat"
        },
        {
            "passage": "Oscar was excited to visit his friend Andrew.",
            "question": "Who was he visiting?",
            "answer": "Andrew"
        }
    ...
    pd.DataFrame([
        ...
        {
            "passage": "Aayush attended the funeral of his best friend James.",
            "question": "Whose funeral?",
            "answer": "James"
        },
        {
            "passage": "The cup was placed on the heavy table.",
    ...
    pd.DataFrame([
        ...
        {
            "passage": "The teacher yelled at Warren’s daughter.",
            "question": "Whose daughter?",
            "answer": "Warren"
        },
        {
            "passage": "The plane landed in San Francisco during the storm.",
            "question": "What landed in San Franscisco?",
            "answer": "The plane"
        }
    ]),
]
...
class WSCDecomp(Decomposition):
    ...
    def get_boost_decomp_examples(self, train_data, boost_id):
        extr_k, conv_k, answ_k = self.k_per_boost
        return [
            extract_relevant_phrase_examples[boost_id][:extr_k],
            convert_reason_to_q_examples[boost_id][:conv_k],
            answer_q_in_passage_examples[boost_id][:answ_k],
        ]

    # def read_data(self, save_dir, overwrite_data):
    #     return load_hf_data(save_dir, self.task_name, self.val_split, "super_glue_wsc", overwrite_data)
    ...
    def question_answer(
        ...
        if saved_extraction:
            relevant_phrase = saved_extraction
        else:
    ...
    def run_decomposed_prompt(
        ...
        for i in range(len(all_boost_preds[0])):
            ...
            individual_precisions.append(classification_report(labels, [p[i] for p in all_boost_preds], output_dict=True)["macro avg"]["precision"])
            individual_recalls.append(classification_report(labels, [p[i] for p in all_boost_preds], output_dict=True)["macro avg"]["recall"])
        report = classification_report(labels, preds, output_dict=True)
        precision = report["macro avg"]["precision"]
        recall = report["macro avg"]["recall"] 
        overall = (report["accuracy"], precision, recall)
        individual = (individual_accuracies, individual_precisions, individual_recalls)
        return expt_log, expt_log_train, overall, individual
    ...
    def _run_decomp_single_data(self, test_data, boost_dfs, manifest, overwrite_manifest, run_limit=-1):
        ...
        for i, (ind, row) in tqdm(
            ...
            for j, boost_examples in enumerate(boost_dfs):
                all_prompts = []
                text = row['text']
                ...
                (
                    ...
                    manifest,
                    overwrite_manifest,
                    self.saved_augmentations[i]["extraction"][j] if self.use_extraction else None,
                    self.saved_augmentations[i]["question"][j] if self.use_question else None,
                )
                ...
                answers_across_boost.append(qa_answer)
            ...
            entry = {
                ...
                "answers_boost": answers_across_boost,
                "gold": gold_answer,
            }
            expt_log[ind] = entry
...
def main():
    ...
    data_dir = "/content/drive/MyDrive/cs330_project/ama_logs/final_runs/super_glue_wsc"
    decomp = WSCDecomp(task_name, data_dir, "data")
    decomp.run(args)
