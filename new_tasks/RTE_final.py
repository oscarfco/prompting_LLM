...
class RTEDecomp(Decomposition):
    ...
    def get_question(self, statement, prompt, boost_ex, manifest, overwrite_manifest, saved_question, saved_proposed_answer):
    ...
    def _run_decomp_single_data(self, test_data, boost_dfs, manifest, overwrite_manifest, run_limit=-1):
        ...
        for i, (ind, row) in tqdm(
            ...
            for boost_num, boost_examples in enumerate(boost_dfs):
                ...
                if boost_num < 4:
                    saved_question = self.saved_augmentations[str(i)]["generated_questions"][boost_num] if self.use_question else None
                    saved_proposed_answer = self.saved_augmentations[str(i)]["proposed_answers"][boost_num] if self.use_question else None
                    question, proposed_answer, question_final_prompt = self.get_question(
                        statement, questioner, boost_examples[0], manifest, overwrite_manifest, saved_question, saved_proposed_answer
                    )
                    generated_questions.append(question)
                    proposed_answers.append(proposed_answer)
                    if i == 0:
                        print("PROMPT:")
