...
DATA_DIR = os.environ.get("AMA_DATA", "/data")
...
def get_args():
    ...
    parser.add_argument(
        "--k_per_chain", type=str, default="3,7,3", help="Number of few shot per chain"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output_metrics_file", type=str, default="/home/ahojel/ama/metrics.json", help="Output file for all metrics."
    )
    parser.add_argument(
        "--save_dir", type=str, default="/content/drive/MyDrive/cs330_project/ama_logs/v3_final_runs", help="Data directory"
    )
    parser.add_argument(
        "--run_decomp",
        type=int,
    ...
    parser.add_argument(
        "--cache_connection",
        type=str,
        default="/content/drive/MyDrive/cs330_project/manifest_log/final_runs.sqlite",
    ...
    #     "--saved_augmentations",
    #     type=str,
    #     default="/home/ahojel/ama_logs/saved_augmentations",
    #     help="path to saved augmentations",
    # )
    parser.add_argument(
        "--augmentations_from",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Use the extraction from saved augmentations",
        choices=[0, 1],
    )
    parser.add_argument(
        "--use_question",
        type=int,
        default=0,
        help="Use the question from saved augmentations",
        choices=[0, 1],
    )
...
class Decomposition:
    def __init__(self, task_name, data_dir, val_split="validation"):
        ...
        self.saved_augmentations = None
        self.augmentation_from = None
        self.use_extraction = False
        self.use_question = False
        self.k_per_boost = None
    ...
    def run(self, args):
        ...
        # add infrastructure for saved augmentations 
        saved_augmentation = f"/home/ahojel/ama_logs/saved_augmentations/{self.task_name}/EleutherAI_{args.augmentations_from}.json"
        if bool(args.use_question) or bool(args.use_extraction):
            f = open(saved_augmentation)
            self.saved_augmentations = json.load(f)
            
        self.use_extraction = bool(args.use_extraction)
        self.use_question = bool(args.use_question)
        self.augmentation_from = args.augmentations_from

        # set up stuff for k-shot automation
        self.k_per_boost = list(map(int,args.k_per_chain.split(',')))
        ...
        if num_run != len(data_test):
            ...
            #save_results = False keep saving the results
        ...
        if bool(args.run_decomp):
            ...
            if self.use_question:
                run_name = f"{model_name}_decomposed_question={self.augmentation_from}_{today}"
            elif self.use_extraction:
                run_name = f"{model_name}_decomposed_extraction={self.augmentation_from}_{today}"
            else:
                run_name = f"{model_name}_k_per_chain={args.k_per_chain}_{today}"
        ...
        if len(metric_decomposed_by_boost) > 0:
            ...
            print("Accuracy by Boost Set Decomposed Average", np.mean(np.array(metric_decomposed_by_boost), axis=1))
        ...
        metrics = {
            ...
            "few_shot": metric_few,
            "k_per_chain": args.k_per_chain,
            "questions_from": args.augmentations_from if bool(args.use_question) else "none",
            "extractions_from": args.augmentations_from if bool(args.use_extraction) else "none",
            "decomposed": metric_decomposed,
            "decomposed_by_boost": metric_decomposed_by_boost,
            "decomposed_by_boost_avg": list(np.mean(np.array(metric_decomposed_by_boost), axis=1)),
        ...
        print(f"Loaded augmentations from {args.augmentations_from}")
