    ...
    def _run_decomp_single_data(
        ...
        for i, (ind, row) in tqdm(
            ...
            for boost_num, boost_examples in enumerate(boost_dfs):
                ...
                if boost_num < 3:
                    saved_question = self.saved_augmentations[i]["question"][boost_num] if self.use_question else None
                    question, question_final_prompt = self.get_question(
                        statement, questioner_prompt, boost_examples[0], manifest, overwrite_manifest, saved_question
                    )
